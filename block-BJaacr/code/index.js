...
for(let i = 0; i < got.houses[0].people.length;i++ ){
  ...
  let description = got.houses[0].people[i].description
...
// 11. Push the name of the houses in an array named `houseNames` and Log the array
 let houseNames = [got.houses[0].name,got.houses[1].name]
...
for(let i = 0; i < got.houses.length; i++){
  for (let j = 0; j < got.houses[i].people.length; j++){
    let name =(got.houses[i].people[j].name)
    console.log(name)
  }
}
...
let allPeople = [];
for (let i = 0; i < got.houses.length; i++){
  for(let j = 0; j < got.houses[i].people[j].length; j++ ){
